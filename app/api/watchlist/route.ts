...
const AddSymbolSchema = z.object({
  ...
  addedPrice:  z.number().min(0).optional(),
})
...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      .from('watchlists')
      .select(`id, name, is_default, created_at, watchlist_items(id, symbol, sort_order, added_at, added_price)`)
      .eq('user_id', user.id)
...
export async function POST(request: NextRequest) {
  try {
    const supabase = await createClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return NextResponse.json({ error: 'Login to use watchlist' }, { status: 401 })
    ...
    const { symbol, watchlistId, addedPrice } = parsed.data
